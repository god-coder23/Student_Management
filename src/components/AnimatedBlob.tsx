...
export default function AnimatedBlob() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const centerY =
        canvas.height / 2 - scrollYRef.current * 0.4 + mouseYRef.current * 30;
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const angle =
          (time + i * ((Math.PI * 2) / 3)) * 0.2 + scrollProgress * Math.PI;
        const mouseInfluence = mouseXRef.current * 0.3;
        const offsetX =
          Math.cos(angle + mouseInfluence) * baseRadius * 0.4 +
          mouseXRef.current * 20;
        const offsetY =
          Math.sin(angle) * baseRadius * 0.4 + mouseYRef.current * 20;
        ...
        for (let j = 0; j < Math.PI * 2; j += 0.04) {
          ...
          const scrollDistortion =
            Math.sin(j + scrollProgress * Math.PI * 2) * 0.08;
        ...
        gradient.addColorStop(
          0,
          `rgba(${color.r}, ${color.g}, ${color.b}, ${Math.max(0.1, baseOpacity)})`,
        );
        gradient.addColorStop(
          0.5,
          `rgba(${color.r}, ${color.g}, ${color.b}, ${Math.max(0.05, midOpacity)})`,
        );
